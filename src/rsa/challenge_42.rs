...
// Python script to fake an RSA signature for any certificate. We find new instances of it every other
// year or so.
//
...
// the modulus after being cubed. The block 00:02 (imagine sufficient zero-padding) can be "encrypted"
// in e=3 RSA; it is simply 00:08.
//
...
// As intended, the ffh bytes in that block expand to fill the whole block, producing a "right-justified"
// hash (the last byte of the hash is the last byte of the message).
//
// There was, 7 years ago, a common implementation flaw with RSA verifiers: they'd verify signatures
// by "decrypting" them (cubing them modulo the public exponent) and then "parsing" them by looking
...
// the padding, you leave open the possibility that instead of hundreds of ffh bytes, you have only
// a few, which if you think about it means there could be squizzilions of possible numbers that
