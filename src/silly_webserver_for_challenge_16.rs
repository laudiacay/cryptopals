...
fn challenge_16_oracle(input_string: String) -> Result<Vec<u8>> {
    ...
    println!("output_bytes_from_oracle, about to get encrypted:\n{:?}", output_bytes);
...
pub fn challenge_16_attack() -> Result<bool> {
    ...
    println!("encrypted: \n{:?}", encrypted);
    let jokes_and_tricks = cryptopal_util::fixed_xor(&cryptopal_util::ascii_to_bytes(";admin=true;")?, &cryptopal_util::ascii_to_bytes("AAAAAAAAAAAA")?);
