...
pub fn do_srp(_email: String, password: String, sent_big_a: BigUint) -> Result<()> {
    // C & S
    ...
    // C
    // Generate K = SHA256(S)
    // S comes out to zero, for 0 and exponents of N = A! This is because S = (A * v**u) ** b % N
    c.k = Some(sha256(&BigUint::zero().to_bytes_be()));
